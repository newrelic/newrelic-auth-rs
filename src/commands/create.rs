    ...
    C: HttpClient,
{
    ...
    pub fn create_l1_system_identity(
        ...
        let iam_client = HttpIAMClient::new(self.http_client.clone(), metadata.clone());
...
mod tests {
    ...
    fn test_create_l1_system_identity_success() {
        ...
        let command = CreateCommand::new(Arc::new(mock_http_client));
    ...
    fn test_create_l2_system_identity_malformed_response() {
        ...
        assert!(
            result.is_err(),
            "Expected deserialization error but got success"
        );
    }
    #[test]
    fn test_create_identity_with_empty_metadata() {
        ...
        let result_l1 = command.create_l1_system_identity(&metadata, token.clone());
        assert!(
            result_l1.is_err(),
