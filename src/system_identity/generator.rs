...
use super::iam_client::{IAMClient, SystemIdentityCreationResponseData};

#[derive(Debug, Clone, Error)]
pub enum SystemIdentityCreationError {
...
where
    ...
    I: IAMClient,
{
    pub fn generate(self) -> Result<SystemIdentity, SystemIdentityCreationError> {
        let token = self
            .token_retriever
            .retrieve()
            .map_err(|e| SystemIdentityCreationError::TokenRetriever(e.to_string()))?;
        let pub_key = self.key_creator.create().map_err(|_| {
            SystemIdentityCreationError::KeyPairCreator(String::from("Could not obtain public key"))
        ...
        let SystemIdentityCreationResponseData { client_id, name } = self
            ...
            .map_err(|e| SystemIdentityCreationError::IAMClient(e.to_string()))?;
