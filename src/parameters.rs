...
pub fn build_proxy_args(proxy_args: Option<ProxyArgs>) -> Result<ProxyConfig, Error> {
    match proxy_args {
        Some(args) => {
            let config_result = ProxyConfig::new(
                args.proxy_url.unwrap_or_default(),
                args.proxy_ca_file.unwrap_or_default(),
                args.proxy_ca_dir.unwrap_or_default(),
            )?;
...
mod tests {
    ...
    fn test_build_proxy_with_args() {
        ...
        let result = build_proxy_args(Some(proxy_args));
        assert!(result.is_ok());
    ...
    fn test_build_proxy_without_args() {
        let result = build_proxy_args(None);
        assert!(result.is_ok());
